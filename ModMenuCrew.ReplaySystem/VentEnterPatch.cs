...
public static class VentEnterPatch
{
	public static void Postfix(PlayerPhysics __instance, int ventId)
	{
		...
		{
			ReplayRecorder.Instance.LogEvent(ReplayEventType.Vent, __instance.myPlayer.PlayerId, (byte)ventId, InteropFix.Cast(((Component)__instance).transform.position), "Entered Vent");
